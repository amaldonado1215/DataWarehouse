alter view vwPACPTCollectionDetail as 

select --distinct --ticket 2396 lauren
	CR.PID,
	CR.DOS,
	...
	--Case
		...
		--as ClaimTotalCharge,
	cc.Total_Charge as ClaimTotalCharge,
...
from collect_detail_3350 as Coll_Det
	
	LEFT OUTER JOIN dbo.claim_entity_3350 AS CE ON Coll_det.claim_seq = CE.claim_seq
	LEFT OUTER JOIN dbo.case_report_3350 AS CR ON CE.patient_id = CR.PID 
	LEFT OUTER JOIN dbo.claim_det_3350 AS CD ON CD.claim_id = Coll_Det.claim_seq 
	--LEFT OUTER JOIN dbo.claim_charges_3350 AS CC ON CC.claim_seq = Coll_Det.claim_seq
													--AND CC.proc_code = Coll_Det.proc_code
	LEFT OUTER JOIN (SELECT patient_id,claim_seq,proc_code,
       sum(Unit_Charge*units) as Total_Charge
      FROM dbo.claim_charges_3350 c
      WHERE Unit_Charge <> 0
      GROUP BY patient_id,claim_seq,proc_code) cc
	  ON CC.claim_seq = Coll_Det.claim_seq AND CC.proc_code = Coll_Det.proc_code
	...
	LEFT OUTER JOIN dbo.vwPA_hl7_Raw AS HL7 ON HL7.patient_id = CR.PID 
	--LEFT OUTER JOIN dbo.denialcodelookup as DCL on coll_det.comment = dcl.auth
	 LEFT OUTER JOIN (SELECT AUTH, min(REASON) as Reason
      FROM dbo.DenialCodeLookup
      GROUP BY AUTH) dcl on coll_det.comment = dcl.auth
--where collected > 0
